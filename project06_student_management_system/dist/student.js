export class Student {
    ...
    coursesEnrolled;
    balance = 0;
    constructor(fname, lname, age, contact, id) {
        ...
        // if id is not provided then generate a unique id
            ...
            this.studentID = Math.floor(Math.random() * 1000) + 1;
        this.firstName = fname;
        this.lastName = lname;
        this.age = age;
        this.coursesEnrolled = [];
        this.contactNumber = contact;
        console.log(`New student with roll number ${this.studentID} has been added successfully`);
    }
    ///////////////functions on student///////////////
    enroll(course) {
        if (this.coursesEnrolled.includes(course))
            return -1;
        else {
    ...
    payFee(fee) {
        if (fee <= this.balance) {
            this.balance -= fee;
            console.log(`Fee paid successfully! Remaining balance for student ${this.firstName} is ${this.balance}.`);
        }
        else
            console.log(`Fee can not be greater than the balance, i.e. ${this.balance}`);
    }
    showStatus(rollNumber) {
