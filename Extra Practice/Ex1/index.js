...
let famousPerson = "Holy Prophet(PBUH)", message = `${famousPerson} said "Seek knowledge from cradle to grave."`;
console.log(message);
// Stripping Names: Store a person’s name, and include some whitespace characters at the beginning and end of the name. Make sure you use each character combination, "\t" and "\n", at least once. Print the name once, so the whitespace around the name is displayed. Then print the name after striping the white spaces.
person_name = "\t\n\t\tBushra Attique\t\n";
console.log(`Name with white spaces = [${person_name}] and without whitespaces= ${person_name.trim()} `);
...
console.log(`${favPeople[0]} can't attend the dinner because he if out of country`);
...
do {
    favPlaces.forEach((element) => {
        if (element.charAt(0) == String.fromCharCode(asciiLower) ||
