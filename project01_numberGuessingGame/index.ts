...
let answer = await inquirer.prompt([
	{
		...
		type: "number",
		message:
			"Please guess the computer generated random number between 1 to 10:",
	},
]);
...
for (let i = 0; i < 10; ++i) {
	if (answer.guessedNumber == random) {
		...
		if (answer.guessedNumber < random)
			console.log(
				"You have guessed a number less than the random number,Try Again!"
			);
		else
			console.log(
				"You have guessed a number greater than the random number,Try Again!"
			);
	}
	if (i != 10) {
